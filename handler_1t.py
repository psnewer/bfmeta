...
class Handler_1T(FH):
    ...
    def get_flag(self):
        ...
        FH.T_std = max(0.0, 1.8 + (FH.forward_goods + FH.backward_goods + FH.balance_overflow) / (FH.limit_value * FH.T_rt) * 400)
        if FH.t_f < FH.t_b:
            FH.t_head = max(FH.t_head, FH.tick_price + FH.step_hard)
        elif FH.t_f > FH.t_b:
            FH.t_head = min(FH.t_head, FH.tick_price + FH.step_hard)
