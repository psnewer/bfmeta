...
class Handler_T2(FH):
    ...
    def get_flag(self):
        ...
        if self.D_std > min(FH.forward_limit,FH.backward_limit):
            if self.current_side == 'forward' and FH.backward_stable_price:
                self.adjust_guide(min(FH.forward_limit,FH.backward_limit))
                self.reset_St()
            elif self.current_side == 'backward' and FH.forward_stable_price:
                self.adjust_guide(min(FH.forward_limit, FH.backward_limit))
                self.reset_St()
    ...
    def adjust_rt(self,D_std):
        self.T_rt = (D_std - self.T_guide) / self.goods_rt
        self.D_std = D_std
